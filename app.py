...
with st.spinner("🔄 Loading the interactive map..."):
    st.title("Deforestation in the Brazilian Amazon Forest - Prototype of Interactive Map")
    ...
    # Map setup
    Map = leafmap.Map(center=[-8, -58], zoom=5, ee=True)
    ...
    '''Warning: The Deforestation layers of this prototype are very heavy so they will be loaded separately through the control
    box on the left, and they will take an extra time to render. To help the map, please toggle off and on all of the 
    available layers''' 
    
    hansen_layers = {
    "Deforestation_10n_080w": clipped_10n_080w,
    "Deforestation_10s_060w": clipped_10s_060w,
    "Deforestation_00n_080w": clipped_00n_080w,
    "Deforestation_10s_070w": clipped_10s_070w,
    "Deforestation_10s_050w": clipped_10s_050w,
    "Deforestation_00n_060w": clipped_00n_060w,
    "Deforestation_10n_070w": clipped_10n_070w,
    "Deforestation_10n_060w": clipped_10n_060w,
    "Deforestation_00n_050w": clipped_00n_050w,
    "Deforestation_00n_070w": clipped_00n_070w,
    "Deforestation_10s_080w": clipped_10s_080w,
}

# Add sidebar multiselect
    selected_tiles = st.sidebar.multiselect(
        "Select deforestation tiles to load:",
        options=list(hansen_layers.keys()),
        default=[]  # start with none loaded
)

# Load only selected layers
    for name in selected_tiles:
        img = hansen_layers[name]
    ...
    #if st.sidebar.checkbox("Show Pasture"):
    Map.add_ee_layer(pasture, vis_params, "Agriculture Pasture")
    #if st.sidebar.checkbox("Show Mining Areas"):
